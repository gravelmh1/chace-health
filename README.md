...
| RENPHO | `renpho://` | 숨김 iframe 으로만 시도 |

**폴백은 두지 않습니다.** 앱이 열리지 않았을 때 App Store 검색이나 애플 홈페이지로
보내면 엉뚱한 곳이 열립니다. 안 열리면 아무 일도 일어나지 않는 편이 낫습니다.

방식이 다른 이유: 건강 앱은 아이폰에 항상 설치돼 있으므로 최상위 이동이 안전하고,
iOS 최신 Safari 에서 iframe 으로 던진 스킴이 무시되는 경우가 있어 이쪽이 실제로
열릴 확률이 높습니다. RENPHO 는 설치 여부를 알 수 없으므로 iframe 으로만 던집니다 —
앱이 없어도 "address is invalid" 오류창이 뜨지 않습니다.
