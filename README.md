...
않고 엉뚱한 곳으로 가는 결과만 나왔습니다. App Store 검색이나 애플 홈페이지로
보내는 폴백도 원하는 동작이 아니었습니다.

그래서 기본값은 **아무것도 하지 않음** 입니다 (`js/config.js` 의 스킴·폴백이 모두 `null`).
열 대상이 없으면 카드는 눌리는 상태로 만들지도 않습니다 — 눌러도 아무 일이 없는 것을
눌리는 것처럼 보이게 하지 않기 위해서입니다.

실기기에서 확실히 동작하는 스킴이 확인되면 `js/config.js` 에 넣으면 됩니다.
값이 있을 때만 카드가 눌리는 상태가 되고, 스킴은 숨김 iframe 으로만 시도해
틀려도 Safari 오류창이 뜨지 않습니다.
